...
c_header="bindings/c/include/wickra_copilot.h"
go_header="bindings/go/include/wickra_copilot.h"
...
cbindgen --config bindings/c/cbindgen.toml --crate wickra-copilot-c --output "$tmp" --quiet

if ! diff -u "$c_header" "$tmp"; then
    echo ""
    echo "ERROR: $c_header is out of sync with cbindgen output."
    echo "Regenerate it with:"
    echo "  cbindgen --config bindings/c/cbindgen.toml --crate wickra-copilot-c --output $c_header"
    exit 1
fi
