...
/**
 * A copilot instance driven by JSON commands, over the Wickra C ABI
 ...
 * Only the deterministic core is exposed; the LLM adapter is never reachable over
 * the C ABI.
 */
public final class Copilot implements AutoCloseable {
    ...
    public Copilot(String specJson) {
        try (Arena arena = Arena.ofConfined()) {
            ...
            if (created.address() == 0) {
                throw new IllegalArgumentException("wickra-copilot: invalid spec");
            }
            this.handle = created;
    ...
    public String command(String cmdJson) {
        if (handle == null) {
            throw new IllegalStateException("copilot is closed");
        }
        try (Arena arena = Arena.ofConfined()) {
            MemorySegment cmd = arena.allocateFrom(cmdJson);
            int len = (int) Native.COMMAND.invokeExact(handle, cmd, MemorySegment.NULL, 0L);
            if (len < 0) {
                throw new IllegalStateException("wickra-copilot: command failed (code " + len + ")");
            }
    ...
    /** Free the native copilot handle. */
    @Override
    public void close() {
        if (handle != null) {
            try {
