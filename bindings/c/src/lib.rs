//! The wickra-copilot C ABI — the hub every C-capable language links against.
//!
//! The surface is deliberately tiny and JSON-shaped, exactly like
//! `Copilot::command_json`: construct a `Copilot` from a spec JSON, drive it with
...
//!
//! 1. Call [`wickra_copilot_command`] with `out = NULL`, `cap = 0` to learn the
...
pub const WICKRA_COPILOT_ERR_UTF8: i32 = -2;
/// A panic was caught at the FFI boundary.
pub const WICKRA_COPILOT_ERR_PANIC: i32 = -3;
...
#[no_mangle]
pub unsafe extern "C" fn wickra_copilot_new(spec_json: *const c_char) -> *mut WickraCopilot {
...
#[no_mangle]
pub unsafe extern "C" fn wickra_copilot_free(handle: *mut WickraCopilot) {
    if !handle.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn wickra_copilot_command(
    handle: *mut WickraCopilot,
    cmd_json: *const c_char,
    out: *mut c_char,
    cap: usize,
) -> i32 {
    if handle.is_null() || cmd_json.is_null() {
        return WICKRA_COPILOT_ERR_NULL;
    }
...
#[no_mangle]
pub extern "C" fn wickra_copilot_version() -> *const c_char {
...
mod tests {
    ...
    fn new_command_free_round_trip() {
        ...
        let len = unsafe { wickra_copilot_command(handle, cmd.as_ptr(), ptr::null_mut(), 0) };
        assert!(len > 0);
    ...
    fn build_context_then_query() {
        ...
        .unwrap();
        let len = unsafe { wickra_copilot_command(handle, feeds.as_ptr(), ptr::null_mut(), 0) };
        let mut buf = vec![0u8; usize::try_from(len).unwrap() + 1];
        unsafe {
            wickra_copilot_command(
                handle,
        ...
        assert!(read_buf(&buf).contains("price_move"));

        let query = CString::new(r#"{"cmd":"query","question":"why did BTC dump"}"#).unwrap();
        let qlen = unsafe { wickra_copilot_command(handle, query.as_ptr(), ptr::null_mut(), 0) };
        let mut qbuf = vec![0u8; usize::try_from(qlen).unwrap() + 1];
        unsafe {
            wickra_copilot_command(
                handle,
                query.as_ptr(),
                qbuf.as_mut_ptr().cast::<c_char>(),
                qbuf.len(),
            );
        }
        assert!(read_buf(&qbuf).contains("tool_calls"));
    ...
    fn bad_command_reports_error_in_band() {
        ...
        let len = unsafe { wickra_copilot_command(handle, bad.as_ptr(), ptr::null_mut(), 0) };
    ...
    fn null_spec_yields_null_handle() {
        let handle = unsafe { wickra_copilot_new(ptr::null()) };
        assert!(handle.is_null());
    }
    ...
    fn null_guards_on_command() {
        ...
        // Null handle.
        let code =
            unsafe { wickra_copilot_command(ptr::null_mut(), cmd.as_ptr(), ptr::null_mut(), 0) };
        assert_eq!(code, WICKRA_COPILOT_ERR_NULL);
        // Null command with a valid handle.
        let spec = CString::new(SPEC).unwrap();
        let handle = unsafe { wickra_copilot_new(spec.as_ptr()) };
        let code = unsafe { wickra_copilot_command(handle, ptr::null(), ptr::null_mut(), 0) };
        assert_eq!(code, WICKRA_COPILOT_ERR_NULL);
        unsafe { wickra_copilot_free(handle) };
    }
    ...
    fn free_null_is_a_noop() {
        unsafe { wickra_copilot_free(ptr::null_mut()) };
    }

    #[test]
    fn version_is_nul_terminated() {
        let p = wickra_copilot_version();
